...
3. Sign in to NugIt and approve the connection. New connections default to saving and reading your whole library. You can switch to save-only or limit reading to one collection in Account settings.
...
Save a test Nug. If you restricted the connection to a collection in Account settings, add the Nug to that collection. Ask “Search NugIt for brand voice, read the matching Nug, and use it to draft a short announcement. Cite the original Nug.” Search is literal; try a few words that actually appear in the Nug.

If reading is not authorized, open NugIt Account settings → AI connections → Change access, select **Save and read**, choose the library or a collection, and **Save access**. No reconnection is required. See [retrieval troubleshooting](../troubleshooting.md).

## Reconnection and reviewer checks

Change the selected collection in [NugIt account settings](https://nugit.ai/account) without reconnecting. Test **Disconnect** separately: new save and read calls must fail until you authenticate again. Confirm saved Nugs show Cursor as the source; treat a different label as a compatibility issue.
...
References: [Cursor MCP](https://cursor.com/docs/mcp), [installation links](https://cursor.com/docs/mcp/install-links), [plugin format and submission](https://cursor.com/docs/reference/plugins). Reviewed 2026-09-19.
